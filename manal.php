...
<nav class="navbar navbar-expand-md navbar-dark bg-dark">
  <a class="navbar-brand" href="#"><?php  echo $_SESSION["m"];?></a>
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      <li class="nav-item active">
        <a class="nav-link" href="manal.php?stat=home">Home <span class="sr-only">(current)</span></a>
      </li>
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu the-ul" aria-labelledby="navbarDropdown">
          <a class="dropdown-item" href="manal.php?stat=approve">Approved</a>
          <hr>
          <a class="dropdown-item" href="manal.php?stat=reject">Rejected</a>
          <hr>
          <a class="dropdown-item" href="manal.php?stat=paid">Paid</a>
          <hr>
          <a class="dropdown-item" href="manal.php?stat=unpaid">Unpaid</a>
      </li>
    </ul>
...
if($stat == "home"){ ?>
<h1>Mrs. Manal Page</h1>
<h1 class="text-center text-dark"> Student's Data (American)</h1>
<br><br>
<div class="">
  ...
  <table class="table-search" id="myTable" style="width:200%;"> 
       ...
       echo '<tr>
        <td> '.$row['id'].' </td><td> '.$row['std_name_en'].' </td><td> '.$row['grade'].'  </td><td> '.$row['gander'].'  </td><td>  '.$row['date_b'].' </td><td> '.$row['age'].' </td><td>  '.$row['rlg'].' </td> 
         ...
         <a href="edit.php?id='.$row["id"].'" class="btn btn-outline-info btn-sm">Edit</a></td></td>
       </tr>';
        }   
       }else {
...
<?php
  ...
  $id = isset($_GET["id"]) && is_numeric ($_GET["id"]) ? intval($_GET["id"]) :0;
    ...
    ?>
    <script>
    window.location="manal.php?stat=home";
    </script>
    <?php
}elseif($stat == "rejec"){
  $id = isset($_GET["id"]) && is_numeric ($_GET["id"]) ? intval($_GET["id"]) :0;
  ...
  <?php
      ...
      <div class="">
        ...
        <table class="table-search" id="myTable" style="width:200%;"> 
             ...
             <?php
             $stmet = $conn->prepare("SELECT * FROM students WHERE approv = 2 AND dep = ?");	
             $stmet->execute(array("American"));
             $rows = $stmet->fetchAll();
             $count = $stmet->rowCount();
             if($count>0){
   ...
   <!-- <div class="count_paid btn btn-primary"><span><?php /* $stmet = $conn->prepare("SELECT COUNT(id) FROM students ");	
           $stmet->execute();
           echo $stmet->fetchColumn(); */?></span><i class="fa fa-user"></i></div>-->
    <br><br>
    <div class="">
      <div class="container tabel_res">
      <table class="table-search" id="myTable" style="width:100%;"> 
           ...
           $stmet->execute(array(3,"American"));
           $rows = $stmet->fetchAll();
           $count = $stmet->rowCount();
           if($count>0){
           foreach($rows as $row){
    ...
    <div class="">
      ...
      <table class="table-search" id="myTable" style="width:100%;"> 
           ...
           <?php
           $stmet = $conn->prepare("SELECT * FROM students WHERE approv = ? AND dep = ?");	
           $stmet->execute(array(1,"American"));
           $rows = $stmet->fetchAll();
           $count = $stmet->rowCount();
           if($count>0){
           foreach($rows as $row){
